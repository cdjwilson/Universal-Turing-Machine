...
newConfigntimeshelp :: (Eq state, Eq tape) => TM input state tape -> [Config state tape] -> [Config state tape]
newConfigntimeshelp tm [] = []
newConfigntimeshelp tm (conf : confs) = newConfigntimeshelp tm confs ++ newConfigs tm conf

newConfigntimes :: (Eq state, Eq tape) => TM input state tape -> [Config state tape] -> Int -> [Config state tape]
newConfigntimes tm conf n | n == 0 = newConfigntimeshelp tm conf
                          | otherwise = newConfigntimes tm (newConfigntimeshelp tm conf) (n-1) ++ conf

getnewConfigntimes :: (Eq state, Eq tape) => Int -> TM input state tape -> [input] -> [Config state tape]
getnewConfigntimes n tm xs | n == 0 = [initialConfig tm xs]
                           | otherwise = newConfigntimes tm [initialConfig tm xs] (n-1)
